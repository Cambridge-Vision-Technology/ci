...
Every gate is closed on head `1d335db`, verified 2026-07-31 after the darwin probe landed.
...
- The normal pull-request checks pass. VERIFIED — run 30625193846 on head `1d335db`, all green: `lints`, both `setup-composite-smoke` platforms, and the full `ci` matrix. All FIVE probe-path jobs, the darwin probe included, correctly report `skipping` on a pull request.
- The pull request links to the successful manual probe run. VERIFIED — dispatch 30625195446 on head `1d335db` is green in EVERY job, including all four probes and `probe-log-credential-audit`. The audit scanned six job logs in full (probe 1 contributes three nested jobs) and found no installation token and no private key.
...
- Probe 4, the self-hosted DARWIN standalone-auth probe, PASSES. All twelve steps succeeded on `macos-arm64-nix-darwin`, in 40 seconds. It proved the same five things on `aarch64-darwin` that probe 3 proves on `x86_64-linux`: no SSH is reachable; private `infra-base` clones over HTTPS on App credentials alone; private `nix-shared` resolves through its `git+ssh://` flake URL, which only the `insteadOf` rewrite can make work without SSH; a private Git-LFS-bearing input fetches AND SMUDGES over the token path, which is the previously unproven `aarch64-darwin` LFS capability; and the credential-free public A/B answers `HTTP error 403` "exceeded its LFS budget" with NO 401 and NO 422 — the latter also re-confirming on-host that the runner still carries upstream Nix 2.35.1.
- The self-heal fired on the DARWIN host in that run and is host-verified. The log shows all four file artifacts removed by exact shape — `<runner HOME>/.netrc`, `<runner HOME>/.git-credentials`, `/tmp/netrc`, and `<runner HOME>/.config/nix/nix.conf` — followed by the checkout-header clear. A read-only SSH check of `admins-mac-mini` afterwards confirms all five legacy artifacts are GONE: those four absent, and `.gitconfig` carrying no bare `store` helper. What `.gitconfig` retains is the job-scoped `store --file=…/_work/_temp/github-org-read-credentials.*` helper and the three `insteadOf` rewrites; the file that helper names is deleted, `_work/_temp` is empty, and the next run's inline pre-checkout stale cleanup removes the rewrites. No token material survives the job on darwin.
- The self-heal is observable in probe 3's log: `authenticate.sh: removed /tmp/netrc, a token-bearing file earlier revisions of this repository left at a fixed path`, followed by the checkout-header clear. Nothing token-bearing survived the job: `_temp` is empty afterwards.
