...
- Only global and system git config apply. `actions/checkout` writes local `.git/config`, which Nix never reads.
...
- Pre-checkout stale cleanup (`http.extraHeader`, `url.*.insteadOf`) stays inline on each surface, because it must run before `actions/checkout` while delegation requires a checkout to exist. This is deliberate, not drift.
